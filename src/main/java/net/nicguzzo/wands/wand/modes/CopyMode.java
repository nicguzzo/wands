        ...
        if (wand.getP1() != null && wand.getP2() != null) {
            {
                ...
                if (ll <= wand.MAX_COPY_VOL) {
                    ...
                    if (!wand.preview && !WandsMod.config.disable_info_messages)
                        Compat.displayClientMessage(wand.player,Compat.literal("Copied: " + cp + " blocks"), false);
                } else {
                    Compat.displayClientMessage(wand.player,Compat.literal("Copy limit reached"), false);
                    //log("max volume");
                }
            }
        }

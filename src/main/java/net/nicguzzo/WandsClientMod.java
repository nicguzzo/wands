...
public class WandsClientMod implements ClientModInitializer {
	...
	public void onInitializeClient() {
		...
		undo_keyBinding = FabricKeyBinding.Builder.create(
				new Identifier("wands", "wand_undo"),
				InputUtil.Type.KEYSYM,
				GLFW.GLFW_KEY_U,
				"Wands"
		).build();
            ...
            if (hitResult != null && hitResult.getType() == HitResult.Type.BLOCK) {
				...
				if(player.inventory.countInInv(item_stack.getItem())>=d || player.abilities.creativeMode){					
					...
					if(	( max_xp_blocks>0 || BLOCKS_PER_XP == 0 || player.abilities.creativeMode) && 
						( block_state.isFullCube(client.world, pos)||is_slab||is_pane||is_fence||is_fence_gate||is_stairs))
					{
						float h=1.0f;
						float y0=0.0f;
						if(slab!=null){
						...
						switch (WandItem.getMode() ){
							...
							case 1:{
								mode1(pos,y0,h,side,bufferBuilder,block_hit,client.world,block_state,bb,player,lim,max_xp_blocks,item_stack,(is_pane||is_fence||is_fence_gate||is_stairs||is_leaves));
							}break;
							default:{								
								WandItem.valid=false;
							}break;
	...
	private static void mode0(BlockPos pos,float y0,float h,Direction side,BufferBuilder bufferBuilder, BlockHitResult block_hit,World world,BlockState block_state,Box bb,int lim){
		float x=pos.getX();
		float y=pos.getY();
		float z=pos.getZ();
		float o=0.01f;		
	...
	private static void mode1(BlockPos pos,float y0,float h,Direction side,BufferBuilder bufferBuilder, BlockHitResult block_hit,World world,BlockState block_state,Box bb,
												ClientPlayerEntity player,int lim,int max_xp_blocks,ItemStack item_stack,boolean dont_check_state){		

...
public class WandsConfig {
    ...
    public void generate_allow_list(List<Block> out, String[] str) {
        ...
        for (String id : str) {
            ...
            if (myid.res != null) {
                #if MC_VERSION >=12111
                //TODO: check this!!
                BuiltInRegistries.ITEM.get(myid.res).map(itemReference -> {
                ...
                #else
                Item item=BuiltInRegistries.ITEM.get(myid.res);
    ...
    public void generate_extra_tool(List<ExtraTool> out, ExtraTool_def[] tools) {
        MyIdExt id = new MyIdExt();
        for (ExtraTool_def tool : tools) {
            id.tryParse(tool.item);
            if (id.res != null) {
                #if MC_VERSION >=12111
                BuiltInRegistries.ITEM.get(id.res).map(itemReference -> {
                ...
                #else
                Item item=BuiltInRegistries.ITEM.get(id.res);
                if(item!=null && item!= Items.AIR){
                    ExtraTool extra_tool=new ExtraTool();

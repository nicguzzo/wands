...
public class Btn extends Wdgt {
    ...
    public MyIdExt texture;
    public int textureWidth;
    public int textureHeight;

    // Background texture (optional - if set, draws this instead of solid color)
    public MyIdExt backgroundTexture;
    public MyIdExt backgroundTextureSelected;  // Used when selected (optional)
    ...
    public Btn(MyIdExt backgroundTex, int bgSize, MyIdExt iconTex, int iconSize, BtnClick onClick) {
        this.width = bgSize;
        this.height = bgSize;
    ...
    public void render(GuiGraphics gui, Font font, int mouseX, int mouseY) {
        ...
        if (shouldDrawBackground) {
            if (useBackgroundTexture && backgroundTexture != null) {
                ...
                MyIdExt bgTex = (selected && backgroundTextureSelected != null) ? backgroundTextureSelected : backgroundTexture;
                if (useNineSlice) {
                    blitNineSlice(gui, bgTex, x, y, width, height, nineSliceBorder, nineSliceTexWidth, nineSliceTexHeight);
                } else {
                    Compat.blit(gui, bgTex, x, y, 0, 0,
        ...
        if (texture != null) {
            ...
            Compat.blit(gui, texture, iconX, iconY, 0, 0,
     ...
     */
    private void blitNineSlice(GuiGraphics gui, MyIdExt tex, int destX, int destY, int destW, int destH, int border, int texW, int texH) {
        ...
        // Top-left corner
        Compat.blit(gui, tex, destX, destY, 0, 0, border, border, texW, texH);
        // Top-right corner
        Compat.blit(gui, tex, destX + destW - border, destY, texW - border, 0, border, border, texW, texH);
        // Bottom-left corner
        Compat.blit(gui, tex, destX, destY + destH - border, 0, texH - border, border, border, texW, texH);
        // Bottom-right corner
        Compat.blit(gui, tex, destX + destW - border, destY + destH - border, texW - border, texH - border, border, border, texW, texH);
     ...
     */
    private void blitStretched(GuiGraphics gui, MyIdExt tex, int destX, int destY, int destW, int destH, int srcX, int srcY, int srcW, int srcH, int texW, int texH) {
        ...
        for (int tileY = 0; tileY < destH; tileY += srcH) {
            ...
            for (int tileX = 0; tileX < destW; tileX += srcW) {
                ...
                Compat.blit(gui, tex, destX + tileX, destY + tileY, srcX, srcY, drawW, drawH, texW, texH);
            }
        }
    }
}

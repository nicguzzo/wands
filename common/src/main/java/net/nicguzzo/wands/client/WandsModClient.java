...
public class WandsModClient {
    ...
    public static void initialize() {
        ...
        NetworkManager.registerReceiver(Side.S2C, Networking.STATE_PACKET, (packet, context) -> {
            ...
            context.queue(() -> {
                if (ClientRender.wand != null) {
                    ...
                    if (ClientRender.wand.mode == WandProps.Mode.DIRECTION) ClientRender.wand.palette.slot = slot;
                    if (xp) {
        ...
        NetworkManager.registerReceiver(Side.S2C, Networking.CONF_PACKET, (packet, context) -> {
        ...
        if (client != null && client.player != null) {
            ...
            if (main_bag || off_bag || main) {
                if (main_bag || off_bag) {
                    ...
                    font.draw(poseStack, "Item: " + Compat.translatable_item_name(bgi).getString(), (int) x, (int) y + y_off + font.lineHeight, 0xffffff);
                    font.draw(poseStack, "Total: " + MagicBagItem.getTotal(s), (int) x, (int) y + y_off + font.lineHeight * 2, 0xffffff);
                }
                if (main) {
                    RenderSystem.enableBlend();
                    RenderSystem.defaultBlendFunc();

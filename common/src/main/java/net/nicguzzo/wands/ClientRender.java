...
//                                RenderSystem.disableBlend();
                                    ...
                                    MCVer.inst.set_render_quads(bufferBuilder);
                ...
                case RECT:
                    ...
                    if (wand.valid) {
                        ...
                        if (wand.block_buffer != null ) {
                            ...
                            for (int a = 0; a < wand.block_buffer.get_length() && a < Wand.MAX_LIMIT; a++) {
                                double x = wand.block_buffer.buffer_x[a];
                                double y = wand.block_buffer.buffer_y[a];
                                double z = wand.block_buffer.buffer_z[a];
                                if (wand.block_buffer.state[a] != null) {
                                    preview_shape = wand.block_buffer.state[a].getShape(client.level, last_pos);
                                    List<AABB> list = preview_shape.toAabbs();
                                    for (AABB aabb : list) {
            ...
            for(Direction dir: dirs) {
                ...
                if (!bake_list.isEmpty()) {
                    for (BakedQuad quad : bake_list) {
                        ...
                        //TextureAtlas atlas=client.getTextureAtlas(TextureAtlas.LOCATION_BLOCKS);//
                        MCVer.inst.set_texture(TextureAtlas.LOCATION_BLOCKS);
                        //MCVer.inst.set_texture(quad.sprite.atlas().location());
                        //RenderSystem.setShaderTexture(0, quad.getSprite().atlas().getId());
                        int[] verts = quad.getVertices();
                        //int n=verts.length/8;
                        ...
                        for (int i = 0; i < 4; ++i) {
                            ...
                            bufferBuilder.vertex(x + vx,y + vy,z + vz).uv(u, v).endVertex();
                        }
                    }
                }
            }
